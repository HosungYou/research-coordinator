...
Diverga v6.6.2 introduces **Multi-CLI Compatibility** - enabling Diverga to work seamlessly across Claude Code, Codex CLI, and OpenCode. This release provides adapter templates and unified installation scripts for all three platforms.
...
> **"하나의 Research Coordinator, 다양한 CLI 플랫폼"**
...
| **Model Routing** | opus/sonnet/haiku | o3/gpt-4/gpt-3.5 | opus/sonnet/haiku |
...
```
Diverga/
├── adapters/                      # NEW: CLI-specific templates
│   ├── AGENTS.md.template         # Codex CLI bootstrap
│   ├── oh-my-opencode.template.json  # OpenCode config
...
│
├── scripts/
│   ├── install-multi-cli.sh       # NEW: Unified installer
│   ├── install.sh                 # Claude Code only
│   ├── install-codex.sh           # Codex CLI only
│   └── install-opencode.sh        # OpenCode only
│
└── skills/                        # SKILL.md format agents
    └── research-agents/
